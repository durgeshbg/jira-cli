...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        let last_item_id: u32 = data.last_item_id + 1;
        data.last_item_id = last_item_id;

        data.epics.insert(last_item_id, epic);
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        let last_item_id = data.last_item_id + 1;
        data.last_item_id = last_item_id;

        data.epics
            .get_mut(&epic_id)
            .ok_or_else(|| anyhow!("epic id could not be found in db"))?
            .stories
            .push(last_item_id);

        data.stories.insert(last_item_id, story);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        match data.epics.get(&epic_id) {
            ...
            None => {
                return Err(anyhow::Error::msg("Invalid epic ID"));
            }
        }
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        let story_index = epic
            .stories
            .iter()
            .position(|v| *v == story_id)
            .ok_or_else(|| anyhow!("story id could not be found in epic vector"))?;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        data.stories
            .get_mut(&story_id)
            .ok_or_else(|| anyhow!("story id could not be found in db"))?
            .status = status;
