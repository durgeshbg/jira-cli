...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for k in epics.keys().sorted() {
            let epic = epics
                .get(k)
                .ok_or_else(|| anyhow!("epic not found for id: {k}"))?;

            let id = get_column_string(&k.to_string(), c.0);
            let name = get_column_string(&epic.name, c.1);
            let status = get_column_string(&epic.status.to_string(), c.2);
        ...
        let action = match input {
            ...
            _ => {
                let parsed_input = input
                    .parse::<u32>()
                    .map_err(|_| anyhow!("invalid epic id"))
                    .ok();
                if let Some(epic_id) = parsed_input {
                    let epic = db_state.epics.get(&epic_id);
                    if let Some(_) = epic {
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id = get_column_string(&self.epic_id.to_string(), c.0);
        let name = get_column_string(&epic.name, c.1);
        let description = get_column_string(&epic.description, c.2);
        let status = get_column_string(&epic.status.to_string(), c.3);
        ...
        for k in stories.keys().sorted() {
            let story = stories
                .get(k)
                .ok_or_else(|| anyhow!("story not found for id: {k}"))?;

            let id = get_column_string(&k.to_string(), c.0);
            let name = get_column_string(&story.name, c.1);
            let status = get_column_string(&story.status.to_string(), c.2);
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        ...
        let action = match input {
            ...
            _ => {
                let parsed_input = input.parse::<u32>().ok();
                if let Some(story_id) = parsed_input {
                    let story = db_state.stories.get(&story_id);
                    if let Some(_) = story {
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id = get_column_string(&self.story_id.to_string(), c.0);
        let name = get_column_string(&story.name, c.1);
        let description = get_column_string(&story.description, c.2);
        let status = get_column_string(&story.status.to_string(), c.3);
