        ...
        match action {
            ...
            Action::UpdateEpicStatus { epic_id } => {
                let status = ((*self.prompts.update_status)())
                    .ok_or_else(|| anyhow!("cannot update status, invalid input"))?;
                self.db.clone().update_epic_status(epic_id, status);
            }
            Action::DeleteEpic { epic_id } => {
                self.db.clone().delete_epic(epic_id);
            }
            Action::CreateStory { epic_id } => {
                let story = (*self.prompts.create_story)();
                self.db.clone().create_story(story, epic_id);
            }
            Action::UpdateStoryStatus { story_id } => {
                let status = (*self.prompts.update_status)()
                    .ok_or_else(|| anyhow!("cannot update story status, invalid input"))?;
                self.db.clone().update_story_status(story_id, status);
            }
            Action::DeleteStory { epic_id, story_id } => {
                self.db.clone().delete_story(epic_id, story_id);
            }
            Action::Exit => {
                self.pages.clear();
            }
